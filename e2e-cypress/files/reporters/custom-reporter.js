...
MochaJUnitReporter.prototype.getTestcaseData = function(test, err) {
  ...
  if(fs.existsSync(logFilePath)) {
    systemOutLines = systemOutLines.concat(fs.readFileSync(logFilePath, 'utf-8'))
  }
  ...
  if (systemOutLines.length > 0) {
    testcase.testcase.push({'system-out': {_cdata: this.removeInvalidCharacters(stripAnsi(systemOutLines.join('\n')))}});
  }
