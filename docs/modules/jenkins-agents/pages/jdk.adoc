...
This agent provides 4 JDKs: `java 11`, `java 17`, `java 21` (default) and `java 25`.
...
To switch the environment to `java 25` the script `use-j25.sh` is provided at the jenkins home folder.
...
```
def status = sh(script: "$HOME/use-j25.sh && ./gradlew clean build --stacktrace --no-daemon && $HOME/use-j21.sh", returnStatus: true)
```
