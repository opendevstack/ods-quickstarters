...
source ${BASH_SOURCE%/*}/../../../ods-core/ods-config/ods-core.env
...
git remote add mockbucket "http://$(urlencode ${CD_USER_ID}):$(urlencode ${CD_USER_PWD})@${BITBUCKET_HOST}/scm/opendevstack/ods-quickstarters.git"
git -c http.sslVerify=false push mockbucket --set-upstream "${HEAD}:${REF}"
git remote remove mockbucket


cd ${BASH_SOURCE%/*}/../../../ods-jenkins-shared-library 
HEAD=$(git rev-parse --abbrev-ref HEAD)
if [ "${HEAD}" = "HEAD" ]; then
    HEAD="cicdtests"
    git checkout -b ${HEAD}
fi
git remote add mockbucket "http://$(urlencode ${CD_USER_ID}):$(urlencode ${CD_USER_PWD})@${BITBUCKET_HOST}/scm/opendevstack/ods-jenkins-shared-library.git"
git -c http.sslVerify=false push mockbucket --set-upstream "${HEAD}:${REF}"
git remote remove mockbucket
cd -